import { PlayStoreReviewAlert } from '@/lib/social/playstore';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { AlertTriangle, Star } from 'lucide-react';
import { Tooltip as UITooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';

interface PlayStoreActionColumnProps {
  alerts: PlayStoreReviewAlert[];
}

const tagTone = (tag: PlayStoreReviewAlert['sentimentTag']) => {
  switch (tag) {
    case 'critical':
...
export function PlayStoreActionColumn({ alerts }: PlayStoreActionColumnProps) {
  return (
    <Card className="bg-gray-900 border-gray-800 flex flex-col min-h-[420px]">
      <CardHeader className="px-6 pt-6 pb-3">
        <CardTitle className="flex items-center gap-2 text-white text-lg">
          <AlertTriangle className="h-5 w-5 text-red-400" />
          Play Store Review Alerts
        </CardTitle>
        <CardDescription className="text-gray-400">
          High-impact Google Play review clusters influencing rating momentum
        </CardDescription>
      </CardHeader>
                ...
                <div className="p-3 rounded-lg border border-gray-800 bg-gray-900/60 hover:bg-gray-900 hover:border-red-500/40 transition-all cursor-pointer">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 space-y-1">
                      ...
                      <p className="text-xs text-gray-300 leading-snug">{alert.reviewSnippet}</p>
                    </div>
              ...
              >
                <div className="space-y-3">
                  ...
                  <div className="text-xs text-gray-400 space-y-2">
                    ...
                    <p>Device context: {alert.deviceContext}</p>
                    <p>Android version: {alert.androidVersion}</p>
                    <div>
                      <h4 className="text-[11px] uppercase tracking-wide text-gray-500">Review content</h4>
                      <p className="text-gray-300 leading-relaxed">{alert.summary.replace(/^“|”$/g, '')}</p>
                    </div>
                  </div>
