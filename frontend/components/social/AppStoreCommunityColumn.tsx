import { AppStoreViralityTopic, APPSTORE_SENTIMENT_LEVELS } from '@/lib/social/appstore';
...
interface AppStoreCommunityColumnProps {
  viralityTopics: AppStoreViralityTopic[];
}
...
const GRADIENT_COLORS: Record<(typeof APPSTORE_SENTIMENT_LEVELS)[number]['key'], string> = {
  level1: '#10b981',
  level2: '#f59e0b',
  level3: '#f97316',
  level4: '#ef4444',
...
export function AppStoreCommunityColumn({ viralityTopics }: AppStoreCommunityColumnProps) {
  ...
  const chartData = useMemo(() => viralityTopics.map(topic => {
    ...
    const breakdown = APPSTORE_SENTIMENT_LEVELS.map(level => {
      ...
      <Card className={SOCIAL_CARD_BASE}>
        <CardHeader>
          ...
          <CardDescription className="text-gray-400">
            Star mix for the most viral App Store review themes this week
          </CardDescription>
        </CardHeader>
            ...
            >
              <defs>
                {sortedChartData.map((topic, index) => (
                  <linearGradient
                    key={topic.name}
                    id={`appstore-topic-gradient-${index}`}
                    x1="0"
                    y1="0"
                    x2="1"
                    y2="0"
              ...
              <Bar dataKey="totalPercent" radius={[8, 8, 8, 8]} isAnimationActive={false}>
                {sortedChartData.map((topic, index) => (
                  <Cell key={`${topic.name}-bar`} fill={`url(#appstore-topic-gradient-${index})`} />
                ))}
